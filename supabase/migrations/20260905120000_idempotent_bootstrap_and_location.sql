-- Migration: Idempotent bootstrap_account with business location support
-- 1. Adds structured latitude and longitude columns to public.businesses
-- 2. Safely replaces public.bootstrap_account to handle:
--    - Case A: Profile does not exist -> creates business, profile, links business_id, adds role
--    - Case B: Profile exists with business_id -> preserves existing business, updates location, returns business_id
--    - Case C: Profile exists with NULL business_id -> checks existing business by owner_id or creates one, links profiles.business_id
-- 3. Idempotently inserts user_roles using ON CONFLICT (user_id, role) DO NOTHING

alter table public.businesses
  add column if not exists latitude double precision,
  add column if not exists longitude double precision;

-- Drop prior 7-parameter signature to prevent overloaded function conflict
...
begin
  ...
  if p_role = 'business' then
    -- CASE B: Profile exists AND profiles.business_id is already populated
    ...
    -- CASE C: Profile exists BUT profiles.business_id is NULL (or invalid)
  ...
  -- Return the actual business_id and role
  return jsonb_build_object('role', p_role, 'business_id', bid);
end; $$;

revoke execute on function public.bootstrap_account(text, public.app_role, text, uuid, text, text, text, text, text, text, text, double precision, double precision) from public, anon;
grant execute on function public.bootstrap_account(text, public.app_role, text, uuid, text, text, text, text, text, text, text, double precision, double precision) to authenticated;
